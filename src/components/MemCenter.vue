    ...
    methods: {
        ...
        getMemData() {
            ...
            xhr.onload = function () {
                ...
                this.lphone = this.lphonef.slice(2);
            }.bind(this);
        },
        upmemdata() {
            if (this.inputpassword != '' && this.inputpassword != this.oldpassword) {
                alert("輸入舊密碼不符");
                return;
            } else if (this.verifyPsw == false && this.newpassword != '') {
                alert("新密碼格式錯誤");
                return;
            } else if (this.newpassword != this.newpasswordA && this.newpassword != '') {
                alert("輸入新密碼兩次不相同");
                return;
                ...
                this.newlocal = this.areaCode + this.lphone;
                var xhr = new XMLHttpRequest();
                ...
                if (this.newpassword == '') {
                    let new_mem_deta = `id=${this.memId}&account=${this.account}&name=${this.name}&email=${this.email}&birthday=${this.birthday}&phone=${this.phone}&localphone=${this.newlocal}&address=${this.address}`;
                    ...
                    let new_mem_deta = `id=${this.memId}&account=${this.account}&password=${this.newpassword}&name=${this.name}&email=${this.email}&birthday=${this.birthday}&phone=${this.phone}&localphone=${this.newlocal}&address=${this.address}`;
                    xhr.send(new_mem_deta);
